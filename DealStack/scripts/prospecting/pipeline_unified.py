...
def get_dashboard_api_key() -> str:
    """Obtem API key para o dashboard. Tenta DASHBOARD_API_KEY primeiro,
    depois extrai do JSON API_KEYS (formato .env), depois fallback dev."""
...
def post_to_dashboard(endpoint: str, data: dict) -> bool:
    ...
    try:
        ...
        if not api_key:
            print(f"   ⚠️ API key não configurada (DASHBOARD_API_KEY ou API_KEYS); sincronização ignorada.")
            return False
