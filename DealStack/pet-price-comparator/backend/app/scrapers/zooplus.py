"""ZooPlus scraper implementation (scaffold)."""
...
class ZooPlusScraper(BaseScraper):
    """Scraper for ZooPlus (zooplus.com.br)."""

    retailer_name = "zooplus"
    base_url = "https://www.zooplus.com.br"
    search_path = "/search"
    rate_limit = 1.0

    SELECTORS = {
        "search_results": ".product-tile, .product-item, .product-card",
        "product_link": "a[href*='/p/'], a.product-link",
        "product_name": ".product-name, .title, h3",
        "product_price": ".price .sales, .price-amount, [data-price]",
        "product_original_price": ".price .original, .rrp-price",
        "product_image": ".product-image img, .image img",
        "product_brand": ".brand, .manufacturer",
        "availability": ".availability, .stock-status, .deliverability",
        "detail_name": "h1.product-name, h1.title, .product-detail-name",
        "detail_price": ".price .sales, .price-amount, [data-price]",
        "detail_original_price": ".price .original, .rrp-price",
        "detail_image": ".product-gallery img, .product-image img",
        "detail_brand": ".brand, .manufacturer",
        "detail_description": ".description, .product-description, .product-details",
        "detail_gtin": ".gtin, .ean, .barcode, .sku",
    }
    ...
    async def _parse_search_card(self, card) -> Optional[ScrapedProduct]:
        try:
            ...
            if avail_elem:
                ...
                if any(word in avail_text.lower() for word in ["indisponível", "esgotado", "out of stock"]):
