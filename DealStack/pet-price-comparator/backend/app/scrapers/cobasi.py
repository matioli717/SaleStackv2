"""Cobasi scraper implementation (scaffold)."""
...
class CobasiScraper(BaseScraper):
    """Scraper for Cobasi (cobasi.com.br)."""

    retailer_name = "cobasi"
    base_url = "https://www.cobasi.com.br"
    search_path = "/busca"
    rate_limit = 1.0

    SELECTORS = {
        "search_results": '[data-testid="product-item-v4"]',
        "product_link": 'self',  # The card itself is an <a> tag
        "product_name": 'h3[class*="body-text-sm"], h3.styles__Text-sc-1xrdqsf-3',
        "product_price": 'span[class*="card-price"], span[class*="subtitle-sm"]',
        "product_original_price": 'span[class*="original-price"], span[class*="strike"]',
        "product_image": 'img.styles__Image-sc-eppxm3-0, img[class*="Image"]',
        "product_brand": '.styles__BrandTextAndStars-sc-1xrdqsf-1 span.caption-sm, span[title][class*="caption"]',
        "availability": ".availability, .stock-status",
        "detail_name": "h1.product-name, h1.title, [data-testid='product-name']",
        "detail_price": 'span[class*="card-price"], span[class*="subtitle-sm"]',
        "detail_original_price": 'span[class*="original-price"], span[class*="strike"]',
        "detail_image": ".product-gallery img, .product-image img",
        "detail_brand": ".brand, .product-brand",
        "detail_description": ".description, .product-description",
        "detail_gtin": ".gtin, .ean, .sku, [data-testid='gtin']",
    }
    ...
    async def _parse_search_card(self, card) -> Optional[ScrapedProduct]:
        try:
            # The card itself is an <a> tag with the product link
            ...
            if avail_elem:
                ...
                if any(word in avail_text.lower() for word in ["indisponível", "esgotado"]):
    ...
    def _extract_external_id(self, url: str) -> Optional[str]:
        import re
        # Cobasi URLs have format: /produto-name/p?idsku=946087
        match = re.search(r"[?&]idsku=(\d+)", url)
        if match:
